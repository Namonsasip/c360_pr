...
def popular_top_up_channel_with_rank(input_df,topup_type_ref):
    ...
    sum(payments_total_top_up) as payments_total_top_up
    from input_df 
    ...
    access_method_num,
    register_date,
    topup_type_ref.recharge_topup_event_type_name order by payments_total_top_up desc""")
    ...
    access_method_num,
    register_date,
    recharge_topup_event_type_name as payments_top_up_channel,
    payments_total_top_up,
    row_number() over(partition by start_of_week,
    access_method_num,
    register_date order by payments_total_top_up desc) as rank from df""")
...
def last_top_up_channel(input_df,topup_type_ref):
    ...
    date(register_date),
    recharge_time,
    recharge_topup_event_type_name as payments_last_top_up_channel,
