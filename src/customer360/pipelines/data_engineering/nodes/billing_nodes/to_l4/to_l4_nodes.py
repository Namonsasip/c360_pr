...
def l4_billing_last_and_most_billing_payment_detail_payment(
        paymentDF
):
    if check_empty_dfs([paymentDF]):
        return get_spark_empty_df()

    paymentDF = paymentDF.withColumn("start_of_month",
                                     f.to_date(f.date_trunc('month', f.to_date((f.col('payment_date'))))))
    ################################# Start Implementing Data availability checks #############################
    paymentDF = data_non_availability_and_missing_check(df=paymentDF, grouping="monthly",
...
def l4_billing_last_and_most_billing_payment_detail_profile(profileDF):

    if check_empty_dfs([profileDF]):
        return get_spark_empty_df()
...
def l4_billing_last_and_most_billing_payment_detail_1(
        paymentDF, profileDF, paymentMst
):
    if check_empty_dfs([paymentDF, profileDF, paymentMst]):
        return get_spark_empty_df(2)

    min_value = union_dataframes_with_missing_cols(
        [
            paymentDF.select(f.max(f.col("start_of_month")).alias("max_date")),
            profileDF.select(f.max(f.col("start_of_month")).alias("max_date"))
        ]
    ).select(f.min(f.col("max_date")).alias("min_date")).collect()[0].min_date

    paymentDF = paymentDF.filter(f.col("start_of_month") <= min_value)
    profileDF = profileDF.filter(f.col("start_of_month") <= min_value)


    if check_empty_dfs([paymentDF, profileDF]):
        return get_spark_empty_df(2)

    paymentDF = paymentDF.withColumn("rn", f.expr(
        "row_number() over (partition by payment_identifier order by partition_date desc)")).where("rn = 1").drop("rn")

    pymtSelectedDF = paymentDF.join(paymentMst, (paymentDF.payment_channel == paymentMst.payment_channel_code))\
    ...
    return [pymtSelectedDF, profileDF]

def l4_billing_last_and_most_billing_payment_detail_2(profileDF):
...
def l4_billing_last_and_most_billing_payment_detail_3(pymtSelectedDF, profileDF):
    if check_empty_dfs([pymtSelectedDF, profileDF]):
        return get_spark_empty_df()
...
def l4_billing_last_and_most_billing_payment_detail_last(pymtDF):

    if check_empty_dfs([pymtDF]):
        return get_spark_empty_df()
...
def l4_billing_last_and_most_billing_payment_detail_most3m(pymtDF):
...
def l4_billing_last_and_most_billing_payment_detail_most6m(pymtDF):
...
def l4_billing_last_and_most_billing_payment_detail_4(lastDF, most6mDF, most3mDF):

    if check_empty_dfs([lastDF, most6mDF, most3mDF]):
        return get_spark_empty_df()
