...
def dac_for_streaming_to_l1_intermediate_pipeline(
        input_df: DataFrame,
        streaming_content_type_features_tbl: str,
        streaming_tv_channel_features_tbl: str,
        cust_df: DataFrame,
):
    ...
    input_content_type_df = data_non_availability_and_missing_check(
        df=input_df, grouping="daily", par_col="partition_date", target_table_name=streaming_content_type_features_tbl)

    input_tv_channel_df = data_non_availability_and_missing_check(
        df=input_df, grouping="daily", par_col="partition_date", target_table_name=streaming_tv_channel_features_tbl)

    cust_df = data_non_availability_and_missing_check(df=cust_df, grouping="daily", par_col="event_partition_date",
                                                       target_table_name=streaming_content_type_features_tbl)

    cust_df = data_non_availability_and_missing_check(df=cust_df, grouping="daily", par_col="event_partition_date",
                                                       target_table_name=streaming_tv_channel_features_tbl)

    if check_empty_dfs([input_content_type_df, input_tv_channel_df, cust_df]):
        return [get_spark_empty_df(), get_spark_empty_df(), get_spark_empty_df()]

    min_value = union_dataframes_with_missing_cols(
        [
            input_content_type_df.select(
                f.max(f.to_date((f.col("partition_date")).cast(StringType()), 'yyyyMMdd')).alias("max_date")),
            input_tv_channel_df.select(
                f.max(f.to_date((f.col("partition_date")).cast(StringType()), 'yyyyMMdd')).alias("max_date")),
            cust_df.select(
    ...
    input_content_type_df = input_content_type_df.filter(f.to_date((f.col("partition_date")).cast(StringType()), 'yyyyMMdd') <= min_value)
    input_tv_channel_df = input_tv_channel_df.filter(f.to_date((f.col("partition_date")).cast(StringType()), 'yyyyMMdd') <= min_value)
    cust_df = cust_df.filter(f.col("event_partition_date") <= min_value)

    ################################# End Implementing Data availability checks ###############################

    return [input_content_type_df, input_tv_channel_df, cust_df]
