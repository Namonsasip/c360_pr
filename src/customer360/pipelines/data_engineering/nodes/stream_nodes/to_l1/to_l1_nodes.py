...
def build_stream_mobile_app_categories_master_table(df_app_categories_master_raw: DataFrame, aib_priority_mapping: DataFrame) -> DataFrame:
    
    df_app_categories_master_clean = (
        df_app_categories_master_raw.withColumn("level_1", f.trim(f.lower(f.col("level_1"))))
    ...
    ).drop(*['group_name']).drop_duplicates()

    cols_to_check = ['application_name','level_1','level_2','level_3','level_4']

    df_app_categories_master_clean_filtered = df_app_categories_master_clean.select(cols_to_check).drop_duplicates()
    total_rows_in_master = df_app_categories_master_clean_filtered.count()
    unique_rows_in_master = df_app_categories_master_clean_filtered.dropDuplicates(subset=["application_name"]).count()
...
def node_join_soc_hourly_with_aib_agg(
    df_soc_app_hourly: pyspark.sql.DataFrame, df_app_categories_master: pyspark.sql.DataFrame
):
    ...
    df_soc_app_hourly_with_iab_raw = df_soc_app_hourly_with_iab_raw.drop(*['application'])
    df_soc_app_hourly_with_iab_raw = df_soc_app_hourly_with_iab_raw.withColumnRenamed('application_name','application')
