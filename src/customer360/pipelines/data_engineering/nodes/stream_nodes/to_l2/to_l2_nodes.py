...
def generate_l2_fav_streaming_day(input_df: DataFrame, app_list: list):
    ...
    for each_app in app_list:
        ...
        ctx.catalog.save(
            "l2_streaming_fav_{}_streaming_day_of_week_feature".format(each_app), df
        )

    return None


def dac_for_streaming_to_l2_pipeline_from_l1(
...
def dac_for_streaming_to_l2_pipeline_from_l2(
    input_df: DataFrame, target_table_name: str
):
    ################################# Start Implementing Data availability checks #############################
    if check_empty_dfs([input_df]):
...
def streaming_to_l2_content_type_features(
    ...
    input_df = data_non_availability_and_missing_check(
        ...
        par_col="event_partition_date",
        missing_data_check_flg="Y",
        target_table_name=l2_streaming_fav_content_group_by_duration_dict[
            "output_catalog"
        ],
        exception_partitions=["2020-04-27"],
    )
    ...
    for curr_item in add_list:
        ...
        int_l2_streaming_content_type_features = node_from_config(
            small_df, int_l2_streaming_content_type_features_dict
        )
        CNTX.catalog.save(
            int_l2_streaming_content_type_features_dict["output_catalog"],
            int_l2_streaming_content_type_features,
        )

        l2_streaming_fav_content_group_by_volume = node_from_config(
            int_l2_streaming_content_type_features,
            l2_streaming_fav_content_group_by_volume_dict,
        )
        CNTX.catalog.save(
            l2_streaming_fav_content_group_by_volume_dict["output_catalog"],
            l2_streaming_fav_content_group_by_volume,
        )

        l2_streaming_fav_content_group_by_duration = node_from_config(
            int_l2_streaming_content_type_features,
            l2_streaming_fav_content_group_by_duration_dict,
        )
        CNTX.catalog.save(
            l2_streaming_fav_content_group_by_duration_dict["output_catalog"],
...
def streaming_to_l2_tv_channel_type_features(
    input_df: DataFrame,
    ...
    input_df = data_non_availability_and_missing_check(
        ...
        par_col="event_partition_date",
        missing_data_check_flg="Y",
        target_table_name=l2_streaming_fav_tv_channel_by_duration_dict[
            "output_catalog"
        ],
        exception_partitions=["2020-04-27"],
    )
    ...
    for curr_item in add_list:
        logging.info("running for dates {0}".format(str(curr_item)))
        small_df = data_frame.filter(F.col("start_of_week").isin(*[curr_item]))
        int_l2_streaming_tv_channel_features = node_from_config(
            small_df, int_l2_streaming_tv_channel_features_dict
        )
        CNTX.catalog.save(
            int_l2_streaming_tv_channel_features_dict["output_catalog"],
            int_l2_streaming_tv_channel_features,
        )

        l2_streaming_fav_tv_channel_by_volume = node_from_config(
            int_l2_streaming_tv_channel_features,
            l2_streaming_fav_tv_channel_by_volume_dict,
        )
        CNTX.catalog.save(
            l2_streaming_fav_tv_channel_by_volume_dict["output_catalog"],
        ...
        CNTX.catalog.save(
            l2_streaming_fav_tv_channel_by_duration_dict["output_catalog"],
    ...
    l2_streaming_fav_tv_channel_by_volume = node_from_config(
        int_l2_streaming_tv_channel_features, l2_streaming_fav_tv_channel_by_volume_dict
    )
...
def streaming_to_l2_tv_channel_features(
    input_df: DataFrame,
    ...
    l2_streaming_fav_video_service_by_visit_count_feature_dict: dict,
    ...
    input_df = data_non_availability_and_missing_check(
        ...
        par_col="event_partition_date",
        missing_data_check_flg="Y",
        target_table_name=l2_streaming_fav_video_service_by_visit_count_feature_dict[
            "output_catalog"
        ],
        exception_partitions=["2020-04-27"],
    )
    ...
    for curr_item in add_list:
        logging.info("running for dates {0}".format(str(curr_item)))
        small_df = data_frame.filter(F.col("start_of_week").isin(*[curr_item]))
        int_l2_streaming_video_service_feature = node_from_config(
            small_df, int_l2_streaming_video_service_feature_dict
        )
        CNTX.catalog.save(
            int_l2_streaming_video_service_feature_dict["output_catalog"],
        ...
        CNTX.catalog.save(
            l2_streaming_fav_video_service_by_download_feature_dict["output_catalog"],
...
def streaming_to_l2_music_service_by_download(
    input_df: DataFrame,
    ...
    input_df = data_non_availability_and_missing_check(
        ...
        par_col="event_partition_date",
        missing_data_check_flg="Y",
        target_table_name=l2_streaming_fav_music_service_by_visit_count_feature_dict[
            "output_catalog"
        ],
        exception_partitions=["2020-04-27"],
    )
    ...
    for curr_item in add_list:
        logging.info("running for dates {0}".format(str(curr_item)))
        small_df = data_frame.filter(F.col("start_of_week").isin(*[curr_item]))
        int_l2_streaming_music_service_feature = node_from_config(
            small_df, int_l2_streaming_music_service_feature_dict
        )
        CNTX.catalog.save(
            int_l2_streaming_music_service_feature_dict["output_catalog"],
        ...
        CNTX.catalog.save(
            l2_streaming_fav_music_service_by_download_feature_dict["output_catalog"],
...
def streaming_to_l2_esoprt_service_by_download(
    input_df: DataFrame,
    ...
    input_df = data_non_availability_and_missing_check(
        ...
        par_col="event_partition_date",
        missing_data_check_flg="Y",
        target_table_name=l2_streaming_fav_esport_service_by_visit_count_feature_dict[
            "output_catalog"
        ],
        exception_partitions=["2020-04-27"],
    )
    ...
    for curr_item in add_list:
        ...
        CNTX.catalog.save(
            int_l2_streaming_esport_service_feature_dict["output_catalog"],
        ...
        CNTX.catalog.save(
            l2_streaming_fav_esport_service_by_download_feature_dict["output_catalog"],
...
def streaming_streaming_ranked_of_day_per_week(
    ...
    for curr_item in add_list:
        logging.info("running for dates {0}".format(str(curr_item)))
        small_df = data_frame.filter(F.col("start_of_week").isin(*[curr_item]))
        int_l2_streaming_sum_per_day = expansion(
            small_df, int_l2_streaming_sum_per_day_dict
        )
        CNTX.catalog.save(
            int_l2_streaming_sum_per_day_dict["output_catalog"],
            int_l2_streaming_sum_per_day,
        )

        int_l2_streaming_ranked_of_day_per_week = node_from_config(
            int_l2_streaming_sum_per_day, int_l2_streaming_ranked_of_day_per_week_dict
        )

        generate_l2_fav_streaming_day(
            int_l2_streaming_ranked_of_day_per_week, streaming_app_list
        )
...
def streaming_to_l2_fav_tv_show_by_share_of_completed_episodes(
    ...
    l2_streaming_fav_tv_show_by_share_of_completed_episodes_dict: dict,
) -> DataFrame:
    """
    ...
    input_df = data_non_availability_and_missing_check(
        ...
        par_col="event_partition_date",
        missing_data_check_flg="Y",
        target_table_name=l2_streaming_fav_tv_show_by_share_of_completed_episodes_dict[
            "output_catalog"
        ],
        exception_partitions=["2020-04-27"],
    )
