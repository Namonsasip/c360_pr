...
def get_matched_urls(df_traffic_join_cp_join_iab: pyspark.sql.DataFrame):
    ...
    ).select("mobile_no", "event_partition_date", "url","category_name", "priority", "total_visit_duration" , "total_visit_count")
...
def l1_digital_get_best_match_for_unmatched_urls(
    ...
    df_traffic_get_missing_urls = (
        ...
        .drop("siteid").select("mobile_no", "event_partition_date", "url","category_name", "priority", "total_visit_duration" , "total_visit_count")
    )
...
def l1_digital_union_matched_and_unmatched_urls(
    ...
    df_traffic_join_cp_matched = df_traffic_join_cp_matched.union(df_traffic_get_missing_urls).groupBy("mobile_no",
                                                                                                       "event_partition_date",
                                                                                                       "url",
                                                                                                       "category_name",
                                                                                                       "priority").agg(
