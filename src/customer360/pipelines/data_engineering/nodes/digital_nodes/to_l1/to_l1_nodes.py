...
def digital_customer_cxense_agg_daily( cxen_traffic:pyspark.sql.DataFrame,cxen_master:pyspark.sql.DataFrame,customer_profile:pyspark.sql.DataFrame,cat_level: dict):
    ...
    cxen_traffic = cxen_traffic.select("mobile_no", "category_name", "total_visit_count","total_visit_duration","event_partition_date")
    #-------- Join Profile ---------#
    cxen_traffic = cxen_traffic.join(customer_profile,on=[cxen_traffic.mobile_no == customer_profile.access_method_num],how="left")
    cxen_traffic = cxen_traffic.select("subscription_identifier","mobile_no", "category_name", "total_visit_count","total_visit_duration",customer_profile.event_partition_date)
    return cxen_traffic
