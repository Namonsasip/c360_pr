...
def l3_digital_mobile_web_category_agg_monthly (mobile_web_daily_agg: DataFrame) -> DataFrame :
    ...
    df_mobile_web_monthly_category_agg = df_mobile_web_monthly.groupBy("subscription_identifier","mobile_no","category_name","start_of_month").agg(
...
def digital_mobile_web_agg_sum_monthly(web_category_agg_daily: pyspark.sql.DataFrame, web_sql: Dict[str, Any]):
...
def digital_mobile_web_category_favorite_monthly_trans(web_category_agg_daily_trans: pyspark.sql.DataFrame,
                                                 aib_clean: pyspark.sql.DataFrame,
                                                 category_level: Dict[str, Any],
                                                 web_sql_transaction: Dict[str, Any]):
    # ---------------  join priority ------------------
    web_category_agg_daily_trans = web_category_agg_daily_trans.join(aib_clean, on=[web_category_agg_daily_trans.category_name == aib_clean[category_level]], how="left")
    web_category_agg_daily_trans = web_category_agg_daily_trans.select(
       web_category_agg_daily_trans["subscription_identifier"],
       web_category_agg_daily_trans["mobile_no"],
       web_category_agg_daily_trans["category_name"],
       web_category_agg_daily_trans["total_visit_count"],
    #    web_category_agg_daily_trans["total_visit_duration"],
    #    web_category_agg_daily_trans["total_volume_byte"],
    #    web_category_agg_daily_trans["total_download_byte"],
    #    web_category_agg_daily_trans["total_upload_byte"],
       web_category_agg_daily_trans["start_of_month"],
       aib_clean["priority"]
    )
    # ---------------  sum traffic ------------------
...
def digital_mobile_web_category_favorite_monthly(web_category_agg_daily_transaction: pyspark.sql.DataFrame,
    ...
    # ---------------  sum cal fav ------------------
    df_return = node_from_config(web_category_agg_daily_transaction, web_sql_transaction)
...
def digital_mobile_web_favorite_by_category_monthly(web_category_agg_monthly: pyspark.sql.DataFrame,aib_clean: pyspark.sql.DataFrame,web_sql_total: Dict[str, Any], web_sql_transaction: Dict[str, Any],web_sql_duration: Dict[str, Any], web_sql_volume: Dict[str, Any],category_level: Dict[str, Any]):
    ...
    web_category_agg_monthly = web_category_agg_monthly.withColumnRenamed("count_trans", 'total_visit_count')
    web_category_agg_monthly = web_category_agg_monthly.withColumnRenamed("duration",'total_visit_duration')
    web_category_agg_monthly = web_category_agg_monthly.withColumnRenamed("total_byte", 'total_volume_byte')
    # ---------------  where data is not true -------------- #
    web_category_agg_monthly = web_category_agg_monthly.where(f.col("count_trans") > 0)
    web_category_agg_monthly = web_category_agg_monthly.where(f.col("duration") > 0)
    web_category_agg_monthly = web_category_agg_monthly.where(f.col("total_byte") > 0)
    web_category_agg_monthly = web_category_agg_monthly.where(f.col("download_byte") > 0)
    web_category_agg_monthly = web_category_agg_monthly.where(f.col("upload_byte") > 0)
    # ---------------  sum traffic by category -------------- #
    web_category_agg_monthly_sql_total = node_from_config(web_category_agg_monthly, web_sql_total)
    
    # ---------------  Join total by category and volumn -------------- #
...
def digital_customer_multi_company_sim_monthly(
    ...
    customer_multi_company_sim = customer_multi_company_sim.select("subscription_identifier","mobile_no", "multi_company_sim_flag", "multi_company_broadband_flag", "multi_company_sim_days", "multi_company_broadband_days","start_of_month")
