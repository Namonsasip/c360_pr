...
def l1_complaints_survey_after_store_visit(
        input_complaints,
        input_cust
):
...
,access_method_num
,round(avg(case when survey_result in ('Very Dissatisfied','Dissatisfied','Neutral','Satisfied','Very Satisfied'
,'ไม่พอใจมาก','ไม่พอใจ','ปานกลาง','พอใจ','พอใจมาก') and location_shop_name_en not like 'Serenade%' 
then getSurveyScoreNumber(survey_result) else null end)) as complaints_avg_csi_shop_score
...
def l1_complaints_ai_chatbot_survey_training(
        input,
        config
):
