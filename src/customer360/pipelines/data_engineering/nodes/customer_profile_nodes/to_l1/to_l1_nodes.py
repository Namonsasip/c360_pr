...
def add_feature_profile_with_join_table2(
    ...
    sql = """
        ...
        """  # remove month_id from partition by
    df = spark.sql(sql)
...
def func_master_table(
    ...
    output_hist = df_hist.filter("cast(prepaid_identn_end_dt as date) > to_date('9999-12-31','yyyy-MM-dd')").select(
        "mobile_no").distinct()
