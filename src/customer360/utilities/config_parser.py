...
def __get_l4_time_granularity_column(read_from):
    ...
    if read_from is None:
        raise ValueError(
            "read_from is mandatory. Please specify either 'l1', 'l2', or 'l3'"
        )
    ...
    raise ValueError(
        "Unknown value for read_from. Please specify either 'l1', 'l2', or 'l3'"
    )
...
def _get_full_data(src_data, fea_dict):
    ...
    if fix_dimension_cols is None or target_dimension_col is None:
        raise ValueError(
            "fix_dimension_cols and target_dimension_cols is mandatory for creating full_data"
        )
    else:
        full_set_1 = spark.sql(
            "select {0},collect_set({1}) as set_1 from src_data {2} group by {0}".format(
                fix_dimension_cols, target_dimension_col, fix_dimension_where_cond
            )
        )
        ...
        full_set_2 = full_set_2.withColumn("set_5", F.split("set_2", ",")).withColumn(
            "set_6", F.split("set_4", ",")
        )
        full_set_2.createOrReplaceTempView("full_set_2")

        full_set_3 = spark.sql(
            "select *,array_distinct(set_5) as set_7, array_distinct(set_6) as set_8 from full_set_2"
        ).drop("set_1", "set_2", "set_3", "set_4", "set_5", "set_6")
        full_set_3.createOrReplaceTempView("full_set_3")

        full_set_4 = spark.sql(
            "select *,array_except(set_8,set_7) as missing_set from full_set_3"
        )
        full_set_4 = full_set_4.drop("set_7", "set_8")
        full_set_4 = full_set_4.withColumn(
            target_dimension_col, explode(full_set_4.missing_set)
        ).drop("missing_set")
...
def l4_rolling_window(input_df: DataFrame, config: dict):
    ...
    for agg_function, column_list in config["feature_list"].items():
        for each_feature_column in column_list:
            if read_from == "l1":
                features.append(
                    "{function}({feature_column}) over ({window}) as {column_name}".format(
                        function=agg_function,
                        feature_column=each_feature_column,
                        window=create_daily_lookback_window(7, config["partition_by"]),
                        column_name="{}_{}_daily_last_seven_day".format(
                            agg_function, each_feature_column
                        ),
                    )
                )
                ...
                features.append(
                    "{function}({feature_column}) over ({window}) as {column_name}".format(
                        ...
                        column_name="{}_{}_daily_last_fourteen_day".format(
                            agg_function, each_feature_column
                        ),
                    )
                )
                ...
                features.append(
                    "{function}({feature_column}) over ({window}) as {column_name}".format(
                        ...
                        column_name="{}_{}_daily_last_thirty_day".format(
                            agg_function, each_feature_column
                        ),
                    )
                )
                ...
                features.append(
                    "{function}({feature_column}) over ({window}) as {column_name}".format(
                        ...
                        column_name="{}_{}_daily_last_ninety_day".format(
                            agg_function, each_feature_column
                        ),
                    )
                )
                ...
                features.append(
                    "{function}({feature_column}) over ({window}) as {column_name}".format(
                        ...
                        column_name="{}_{}_weekly_last_week".format(
                            agg_function, each_feature_column
                        ),
                    )
                )
                ...
                features.append(
                    "{function}({feature_column}) over ({window}) as {column_name}".format(
                        ...
                        column_name="{}_{}_weekly_last_two_week".format(
                            agg_function, each_feature_column
                        ),
                    )
                )
                ...
                features.append(
                    "{function}({feature_column}) over ({window}) as {column_name}".format(
                        ...
                        column_name="{}_{}_weekly_last_four_week".format(
                            agg_function, each_feature_column
                        ),
                    )
                )
                ...
                features.append(
                    "{function}({feature_column}) over ({window}) as {column_name}".format(
                        function=agg_function,
                        feature_column=each_feature_column,
                        window=create_weekly_lookback_window(
                            12, config["partition_by"]
                        ),
                        column_name="{}_{}_weekly_last_twelve_week".format(
                            agg_function, each_feature_column
                        ),
                    )
                )
                ...
                features.append(
                    "{function}({feature_column}) over ({window}) as {column_name}".format(
                        function=agg_function,
                        feature_column=each_feature_column,
                        window=create_monthly_lookback_window(
                            1, config["partition_by"]
                        ),
                        column_name="{}_{}_monthly_last_month".format(
                            agg_function, each_feature_column
                        ),
                    )
                )
                ...
                features.append(
                    "{function}({feature_column}) over ({window}) as {column_name}".format(
                        ...
                        feature_column=each_feature_column,
                        window=create_monthly_lookback_window(
                            3, config["partition_by"]
                        ),
                        column_name="{}_{}_monthly_last_three_month".format(
                            agg_function, each_feature_column
                        ),
                    )
                )

    sql_stmt = sql_stmt.format(",\n".join(features), config.get("where_clause", ""))
...
def create_daily_lookback_window(
    num_of_days, partition_column, order_by_column="event_partition_date"
):
    """
...
def create_monthly_lookback_window(
    num_of_month, partition_column, order_by_column="start_of_month"
):
    """
...
def create_weekly_lookback_window(
    num_of_week, partition_column, order_by_column="start_of_week"
):
    """
...
def create_window_statement(
    partition_column, order_by_column, start_interval, end_interval
):
    """
...
def __generate_l4_rolling_ranked_column(input_df, config) -> DataFrame:
    """
    ...
    if read_from == "l1":
        features.append(
            ...
            """.format(
                ...
                order_column="{}_daily_last_seven_day".format(
                    config["order_by_column_prefix"]
                ),
                order=order,
            )
        )
        ...
        features.append(
            ...
            """.format(
                partition_column=",".join(config["partition_by"]),
                order_column="{}_daily_last_fourteen_day".format(
                    config["order_by_column_prefix"]
                ),
                order=order,
            )
        ...
        features.append(
            ...
            """.format(
                partition_column=",".join(config["partition_by"]),
                order_column="{}_daily_last_thirty_day".format(
                    config["order_by_column_prefix"]
                ),
                order=order,
            )
        ...
        features.append(
            ...
            """.format(
                partition_column=",".join(config["partition_by"]),
                order_column="{}_daily_last_ninety_day".format(
                    config["order_by_column_prefix"]
                ),
                order=order,
            )
        ...
        features.append(
            ...
            """.format(
                partition_column=",".join(config["partition_by"]),
                order_column="{}_weekly_last_week".format(
                    config["order_by_column_prefix"]
                ),
                order=order,
            )
        ...
        features.append(
            ...
            """.format(
                partition_column=",".join(config["partition_by"]),
                order_column="{}_weekly_last_two_week".format(
                    config["order_by_column_prefix"]
                ),
                order=order,
            )
        ...
        features.append(
            ...
            """.format(
                partition_column=",".join(config["partition_by"]),
                order_column="{}_weekly_last_four_week".format(
                    config["order_by_column_prefix"]
                ),
                order=order,
            )
        ...
        features.append(
            ...
            """.format(
                partition_column=",".join(config["partition_by"]),
                order_column="{}_weekly_last_twelve_week".format(
                    config["order_by_column_prefix"]
                ),
                order=order,
            )
        ...
        features.append(
            ...
            """.format(
                partition_column=",".join(config["partition_by"]),
                order_column="{}_monthly_last_month".format(
                    config["order_by_column_prefix"]
                ),
                order=order,
            )
        ...
        features.append(
            ...
            """.format(
                partition_column=",".join(config["partition_by"]),
                order_column="{}_monthly_last_three_month".format(
                    config["order_by_column_prefix"]
                ),
                order=order,
            )
    ...
    sql_stmt = sql_stmt.format(
        ",\n".join(set(features)), config.get("where_clause", "")
    )
...
def __construct_null_safe_join_condition(
    column_list, left_alias="left", right_alias="right"
):
    """
    ...
    :return:
    """
    join_condition = F.col("{}.{}".format(left_alias, column_list[0])).eqNullSafe(
        F.col("{}.{}".format(right_alias, column_list[0]))
    )
    ...
    for each_col in column_list[1:]:
        join_condition &= F.col("{}.{}".format(left_alias, each_col)).eqNullSafe(
            F.col("{}.{}".format(right_alias, each_col))
        )
...
def join_l4_rolling_ranked_table(result_df, config):
    ...
    feature_column = [
        F.col("left.{}".format(each_col)) for each_col in config["partition_by"]
    ]
    ...
    for window_range, df in result_df.items():
        ...
        for each_feature_column in config["feature_column"]:
            feature_column.append(
                F.col(each_feature_column).alias(
                    "{}_{}".format(each_feature_column, window_range)
                )
            )
        ...
        final_df = (
            final_df.alias("left")
            .join(other=df.alias("right"), on=join_condition, how="inner")
...
def __generate_l4_filtered_ranked_table(ranked_df, config):
    """
    ...
    if read_from == "l1":
        result_df["last_seven_day"] = ranked_df.where(
            F.col("daily_rank_last_seven_day") == rank
        )
        result_df["last_fourteen_day"] = ranked_df.where(
            F.col("daily_rank_last_fourteen_day") == rank
        )
        result_df["last_thirty_day"] = ranked_df.where(
            F.col("daily_rank_last_thirty_day") == rank
        )
        result_df["last_ninety_day"] = ranked_df.where(
            F.col("daily_rank_last_ninety_day") == rank
        )

    elif read_from == "l2":
        result_df["last_week"] = ranked_df.where(F.col("weekly_rank_last_week") == rank)
        result_df["last_two_week"] = ranked_df.where(
            F.col("weekly_rank_last_two_week") == rank
        )
        result_df["last_four_week"] = ranked_df.where(
            F.col("weekly_rank_last_four_week") == rank
        )
        result_df["last_twelve_week"] = ranked_df.where(
            F.col("weekly_rank_last_twelve_week") == rank
        )
    else:
        result_df["last_month"] = ranked_df.where(
            F.col("monthly_rank_last_month") == rank
        )
        result_df["last_three_month"] = ranked_df.where(
            F.col("monthly_rank_last_three_month") == rank
        )
...
def l4_rolling_ranked_window(input_df, config) -> Dict[str, DataFrame]:
    """
