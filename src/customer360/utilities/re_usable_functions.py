...
def union_dataframes_with_missing_cols(df_input_or_list, *args):
    ...
    def add_missing_cols(dataframe, col_list):
        missing_cols = [
            column for column in col_list if column not in dataframe.columns
        ]
...
def _l1_join_with_customer_profile(
    input_df, cust_profile_df, config, current_item
) -> DataFrame:
    """
    ...
    cust_profile_col_to_select = list(
        config["join_column_with_cust_profile"].keys()
    ) + [
        "start_of_week",
        "start_of_month",
        "access_method_num",
    ...
    cust_profile_col_to_select = list(
        set(cust_profile_col_to_select)
    )  # remove duplicates

    if not isinstance(current_item[0], datetime):
        current_item = list(
            map(lambda x: datetime.strptime(str(x), "%Y%m%d"), current_item)
        )

    # push down the filter to customer profile to reduce the join rows
    filtered_cust_profile_df = cust_profile_df.filter(
        F.col("event_partition_date").isin(current_item)
...
def _l2_join_with_customer_profile(
    ...
    cust_profile_col_selection = set(
        list(config["join_column_with_cust_profile"].keys())
        + ["subscription_identifier"]
    )
    # grouping all distinct customer per week
    filtered_cust_profile_df = (
        cust_profile_df.filter(F.col("start_of_week").isin(current_item))
...
def _l3_join_with_customer_profile(
    ...
    config["join_column_with_cust_profile"]["start_of_month"] = config[
    ...
    filtered_cust_profile_df = (
        cust_profile_df.withColumnRenamed("partition_month", "start_of_month")
        .filter(
            F.col("start_of_month").isin(current_item)
...
def _join_with_filtered_customer_profile(
    ...
    for left_col, right_col in config["join_column_with_cust_profile"].items():
        condition = F.col("left.{}".format(left_col)).eqNullSafe(
            F.col("right.{}".format(right_col))
        )
    ...
    result_df = filtered_cust_profile_df.alias("left").join(
        other=input_df.alias("right"), on=joined_condition, how="left"
    )
    ...
    for col in input_df.columns:
        if (
            col in filtered_cust_profile_df.columns
            or col in config["join_column_with_cust_profile"].values()
        ):
            continue
...
def _massive_processing(
    ...
    for curr_item in add_list:
        ...
        if cust_profile_df is not None:
            output_df = cust_profile_join_func(
                input_df=output_df,
    ...
    if cust_profile_df is not None:
        return_df = cust_profile_join_func(
            input_df=return_df,
            cust_profile_df=cust_profile_df,
            config=config,
...
def l1_massive_processing(input_df, config, cust_profile_df=None) -> DataFrame:
    """
...
def l2_massive_processing(input_df, config, cust_profile_df=None) -> DataFrame:
    """
...
def l2_massive_processing_with_expansion(
    input_df, config, cust_profile_df=None
) -> DataFrame:
    """
...
def l3_massive_processing(input_df, config, cust_profile_df=None) -> DataFrame:
    """
...
def data_non_availability_and_missing_check(
    df,
    grouping,
    par_col,
    target_table_name,
    ...
    if run_mode is not None:
        if (
            (run_mode.upper() == "UNIT_TEST")
            | (run_mode.upper() == "LOCAL_FS")
            | (run_mode.upper() == "NO")
        ):
            logging.info(
                "Exiting data_non_availability_and_missing_check as OS ENV RUN_MODE  = {}".format(
                    run_mode
                )
            )
            return df
    logging.info(
        "Executing data_non_availability_and_missing_check IMP: OS ENV RUN_MODE NO UNIT TEST CASE SELECTED"
    )
    spark = get_spark_session()

    if running_environment.lower() == "on_premise":
        mtdt_tbl = spark.read.parquet(
            "/projects/prod/c360/data/UTILITIES/metadata_table"
        )
    else:
        mtdt_tbl = spark.read.parquet(
            "/mnt/customer360-blob-output/C360/UTILITIES/metadata_table"
        )
    ...
    tgt_max_date = spark.sql(
        ...
        from mtdt_tbl where table_name = '{0}'""".format(
            target_table_name
        )
    )
    ...
    if grouping.lower() == "daily":
        ...
        if par_col.lower() == "partition_date":
            ...
            src_min_date = spark.sql(
                "select to_date(cast(min({0}) as String),'yyyyMMdd') as src_min_date from df".format(
                    par_col
                )
            )
        else:
            actual_src_partitions = spark.sql(
                "select distinct({0}) as actual_src_partitions from df".format(par_col)
            )
            src_max_date = spark.sql(
            ...
            src_min_date = spark.sql(
                "select min({0}) as src_min_date from df".format(par_col)
            )

        # Creating Check Matrix:
        tgt_max_date = tgt_max_date.crossJoin(src_min_date)
        tgt_max_date = (
            tgt_max_date.withColumn(
                "tgt_max_date",
                F.when(
                    tgt_max_date.tgt_max_data_load_date == "1970-01-01",
                    F.date_sub(tgt_max_date.src_min_date, 1),
                ).otherwise(tgt_max_date.tgt_max_data_load_date),
            )
        ...
        check_matrix = check_matrix.withColumn(
            "No_of_actual_partitions",
            F.datediff(F.col("src_max_date"), F.col("tgt_max_date")),
        ).withColumn(
            "data_partition_missing_flag",
            F.expr(
                "CASE WHEN No_of_actual_partitions = No_of_partitions_in_src THEN 'N' ELSE 'Y' END"
            ),
        )
        ...
        data_partition_missing_flag = (
            check_matrix.select("data_partition_missing_flag")
            .collect()[0]
            .data_partition_missing_flag
        )
        if data_partition_missing_flag == "Y":
            logging.info("checking for missing partition")
            actual_total_partitions = check_matrix.select(
                "No_of_actual_partitions", "tgt_max_date"
            ).withColumn(
                "repeat", F.expr("split(repeat(',', No_of_actual_partitions), ',')")
            )
            actual_total_partitions = (
                actual_total_partitions.select(
                    "*", F.posexplode("repeat").alias("daysToAdd", "dummy")
                )
                .drop("repeat", "dummy", "No_of_actual_partitions")
                .withColumn(
                    "actual_total_partitions",
                    F.expr("date_add(tgt_max_date, daysToAdd)"),
                )
                .filter("daysToAdd != 0")
                .drop("daysToAdd", "tgt_max_date")
            )

            missing_partitions = actual_total_partitions.join(
                actual_src_partitions,
            ...
            if exception_partitions is None or exception_partitions == []:
                ...
                missing_partitions = missing_partitions.filter(
                    (F.col("actual_src_partitions").isNull())
                ).withColumnRenamed("actual_total_partitions", "missing_partitions")
            else:
                logging.info(
                    "exception_partitions found as: {}".format(exception_partitions)
                )
            ...
            logging.info(
                "Data is not found in source for these missing partitions: {}".format(
                    missing_partitions.select("missing_partitions").collect()
                )
            )
            ...
            min_missing_partition = (
                missing_partitions.select(
                    F.min(F.col("missing_partitions")).alias("min_missing_partition")
                )
                .collect()[0]
                .min_missing_partition
            )
            ...
            if min_missing_partition is None:
                logging.info(
                    "No missing partitions found after considering the exception partitions"
                )
                # print("No missing partitions found after excemption check")
            else:
                logging.info(
                    "Getting the data before missing partitions from source dataframe for further processing"
                )
                if par_col.lower() == "partition_date":
                    df = df.filter(
                        F.to_date(F.col(par_col).cast(StringType()), "yyyyMMdd")
                        < min_missing_partition
                    )
                else:
    ...
    ############################################################# Weekly level check ########################################################
        ...
        if par_col == "partition_date":
            ...
            df.createOrReplaceTempView("df")
            actual_src_partitions = spark.sql(
                "select distinct(start_of_week_new) as actual_src_partitions from df"
            )
        else:
            df = df.withColumn(
                "start_of_week_new", F.to_date(F.date_trunc("week", F.col(par_col)))
            )
            df.createOrReplaceTempView("df")
            actual_src_partitions = spark.sql(
                "select distinct(start_of_week_new) as actual_src_partitions from df".format(
                    par_col
                )
            )
        ...
        tgt_max_date = tgt_max_date.crossJoin(
            actual_src_partitions.select(
                F.min(F.col("actual_src_partitions")).alias("src_min_week_partition")
            )
        )
        tgt_max_date = (
            tgt_max_date.withColumn(
                "tgt_max_date",
                F.when(
                    F.col("tgt_max_data_load_date") == "1970-01-01",
                    F.date_sub(F.col("src_min_week_partition"), 1),
                ).otherwise(
                    F.date_sub(F.date_add(F.col("tgt_max_data_load_date"), 7), 1)
                ),
            )
            .drop("tgt_max_data_load_date")
            .drop("src_min_week_partition")
        )

        check_matrix = tgt_max_date.crossJoin(
            actual_src_partitions.select(
                F.max(F.col("actual_src_partitions")).alias("src_max_week_partition"),
        ...
        if data_partition_missing_flag == "Y":
            logging.info(
                "checking for missing weekly partition because data_partition_missing_flag in check matrix is Y"
            )
            actual_total_partitions = check_matrix.select(
                "No_of_actual_partitions",
            ...
            actual_total_partitions = (
                ...
                .withColumn(
                    "actual_total_partitions",
                    F.expr("date_add(tgt_max_date, daysToAdd*7)"),
                )
                .filter("daysToAdd != 0")
                .drop("daysToAdd", "tgt_max_date")
            )
            ...
            logging.info(
                "actual weekly partitions in source data required: {}".format(
                    actual_total_partitions.collect()
                )
            )
            missing_partitions = actual_total_partitions.join(
                actual_src_partitions,
            ...
            logging.info(
                "Data is not found in source for these weekly partitions: {}".format(
                    missing_partitions.select("missing_partitions").collect()
                )
            )
            ...
            if min_missing_partition is None:
                ...
                logging.info(
                    "Getting the data before missing weekly partitions from source dataframe for further processing"
                )
        ...
        if missing_data_check_flg.upper() == "Y":
            logging.info(
                "checking for missing data partitions within weekly partition because missing_data_check_flg is Y"
            )
            ...
            if exception_partitions is None or exception_partitions == []:
                ...
                missing_data_partition = (
                    missing_data_partition.filter(
                        F.col("count_of_data_partitions") != 7
                    )
                    .select(
                        F.min(F.col("start_of_week_new")).alias("start_of_week_new")
                    )
                    .collect()[0]
                    .start_of_week_new
                )
                ...
                logging.info(
                    "Exception partition found as : {}".format(exception_partitions)
                )
                missing_data_partition = (
                    missing_data_partition.filter(
                        (F.col("count_of_data_partitions") != 7)
                        & (~F.col("start_of_week_new").isin(exception_partitions))
                    )
                    .select(
                        F.min(F.col("start_of_week_new")).alias("start_of_week_new")
                    )
            ...
            if (
                missing_data_partition is None
                or missing_data_partition == []
                or missing_data_partition == ""
            ):
                logging.info(
                    "No missing data partitions found within weekly partitions"
                )
                df = df
            else:
                logging.info(
                    "Few data partitions are not found in source data for these weekly partitions: {}".format(
                        missing_data_partition
                    )
                )
                logging.info(
            ...
            logging.info(
                "Skipping missing_data_partition check because missing_data_partition_flg is not 'Y'"
            )

        logging.info(
            "Returning the dataframe after all data availability weekly level checks"
        )
    ...
    ############################################################# Monthly level check ########################################################
        ...
        if par_col == "partition_date":
            ...
            df.createOrReplaceTempView("df")
            actual_src_partitions = spark.sql(
                "select distinct(start_of_month_new) as actual_src_partitions from df"
            )
            ...
            df = df.withColumn(
                ...
                F.to_date(
                    F.date_trunc(
                        "month",
                        F.to_date((F.col(par_col)).cast(StringType()), "yyyyMM"),
                    )
                ),
            )
            df.createOrReplaceTempView("df")
            ...
            df = df.withColumn(
                "start_of_month_new", F.to_date(F.date_trunc("month", F.col(par_col)))
            )
            df.createOrReplaceTempView("df")
            actual_src_partitions = spark.sql(
                "select distinct(start_of_month_new) as actual_src_partitions from df".format(
                    par_col
                )
            )
        ...
        tgt_max_date = tgt_max_date.crossJoin(
            actual_src_partitions.select(
                F.min(F.col("actual_src_partitions")).alias("src_min_month_partition")
            )
        )
        tgt_max_date = (
            tgt_max_date.withColumn(
                ...
                F.when(
                    F.col("tgt_max_data_load_date") == "1970-01-01",
                    F.date_sub(F.col("src_min_month_partition"), 1),
                ).otherwise(
                    F.date_sub(F.add_months(F.col("tgt_max_data_load_date"), 1), 1)
                ),
            )
            .drop("tgt_max_data_load_date")
            .drop("src_min_week_partition")
        ...
        check_matrix = tgt_max_date.crossJoin(
            actual_src_partitions.select(
                F.max(F.col("actual_src_partitions")).alias("src_max_month_partition"),
                F.countDistinct(F.col("actual_src_partitions")).alias(
                    "No_of_partitions_in_src"
                ),
            )
        ...
        check_matrix = check_matrix.withColumn(
            ...
            F.ceil(
                F.months_between(
                    F.col("src_max_month_partition"), F.col("tgt_max_date")
                )
            ),
        ).withColumn(
            "data_partition_missing_flag",
        ...
        if data_partition_missing_flag == "Y":
            logging.info(
                "checking for missing monthly partition because data_partition_missing_flag in check matrix is Y"
            )
            actual_total_partitions = check_matrix.select(
                "No_of_actual_partitions",
            ...
            actual_total_partitions = (
                actual_total_partitions.select(
                    "*", F.posexplode("repeat").alias("monthsToAdd", "dummy")
                )
                .drop("repeat", "dummy", "No_of_actual_partitions")
                .withColumn(
                    "actual_total_partitions",
                    F.expr("add_months(tgt_max_date, monthsToAdd)"),
                )
                .filter("monthsToAdd != 0")
                .drop("monthsToAdd", "tgt_max_date")
            )

            logging.info(
                "actual monthly partitions in source data: {}".format(
                    actual_total_partitions.collect()
                )
            )
            ...
            logging.info(
                "Data is not found in source for these monthly partitions: {}".format(
                    missing_partitions.select("missing_partitions").collect()
                )
            )
            # print("missing partitions:", missing_partitions.select("missing_partitions").collect())
            ...
            if min_missing_partition is None:
                logging.info(
                    "No missing monthly partitions found after considering exception partitions"
                )
                # print("No missing partitions found after excemption check")
            else:
                logging.info(
        ...
        if missing_data_check_flg.upper() == "Y":
            ...
            missing_data_partition = missing_data_partition.withColumn(
                "count_of_actual_data_partitions",
                F.datediff(
                    F.add_months(F.col("start_of_month_new"), 1),
                    F.col("start_of_month_new"),
                ),
            )
            if exception_partitions is None or exception_partitions == []:
                logging.info("No exception_partitions found")
                ...
                missing_data_partition = (
                    missing_data_partition.filter(
                        F.col("count_of_src_data_partitions")
                        != F.col("count_of_actual_data_partitions")
                    )
                    .select(
                        F.min(F.col("start_of_month_new")).alias("start_of_month_new")
                    )
                    .collect()[0]
                    .start_of_month_new
                )
                ...
                logging.info(
                    "Exception partitions found as : {}".format(exception_partitions)
                )
                ...
                missing_data_partition = (
                    missing_data_partition.filter(
                        ...
                        & (~F.col("start_of_month_new").isin(exception_partitions))
                    )
                    .select(
                        F.min(F.col("start_of_month_new")).alias("start_of_month_new")
                    )
            ...
            if (
                ...
                logging.info(
                    "No missing data partitions found within monthly partitions"
                )
        ...
        logging.info(
            "Returning the dataframe after all data availability monthly level checks"
        )
