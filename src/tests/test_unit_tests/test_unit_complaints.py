...
global dummy_list_l0_usage_call
...
global df_l0_usage_call
df_l0_usage_call = spark.createDataFrame(rdd1,schema=StructType([
...
global dummy_list_l0_complaints_acc_atsr
...
global df_l0_ucomplaints_acc_atsr
df_l0_ucomplaints_acc_atsr = spark.createDataFrame(rdd2, schema=StructType([
    StructField("qsc_1", StringType(), True),
    StructField("mobile_no", StringType(), True),
    StructField("cldate", DateType(), True),
]))

global dummy_list_l0_acc_ai_chatbot
...
global df_l0_acc_ai_chatbot
df_l0_acc_ai_chatbot = spark.createDataFrame(rdd3, schema=StructType([
    StructField("qsc_1", StringType(), True),
    StructField("mobile_number", StringType(), True),
    StructField("request_datetime", DateType(), True),
]))

global dummy_list_l0_acc_qmt_csi
...
global df_l0_acc_qmt_csi
df_l0_acc_qmt_csi = spark.createDataFrame(rdd4, schema=StructType([
...
class TestUnitComplaints:
    ...
    def test_l2(self, project_context):
        ...
        assert \
            l2_feature_2.where \
                    (
            "access_method_num = 'mv6fLn5EbNGOgfj8olB9yHg6r8rQRjv92iiccMAtQH71LOCmupE.lEk9eaS1ZyDw' and "
            "start_of_week='2020-01-27'").select("record_count").collect()[0][0] == 2
    ...
    def test_l4(self, project_context):
        ...
        assert \
        l4_feature_1.where("start_of_week='2020-01-27'").select("sum_call_to_dtac_count_weekly_last_week") \
            .collect()[0][0] == None
        assert \
        l4_feature_2.where \
                (
                "access_method_num = 'mv6fLn5EbNGOgfj8olB9yHg6r8rQRjv92iiccMAtQH71LOCmupE.lEk9eaS1ZyDw' and "
                "start_of_week='2020-01-27'") \
            .select("avg_nps_last_week").collect()[0][0] == None
