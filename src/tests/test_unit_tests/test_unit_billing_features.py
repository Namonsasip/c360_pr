...
class TestUnitBilling:
    ...
    def test_topup_and_volume_features(self, project_context):
        ...
        assert \
            daily_topup_volume.where("event_partition_date = '2020-01-01'").select("payments_top_up_volume").collect()[
                0][
                0] == 1
    ...
    def test_arpu_roaming(self, project_context):
        ...
        assert \
            daily_arpu_roaming.where("event_partition_date = '2020-01-01'").select("payments_arpu_roaming").collect()[
                0][
                0] == 1
    ...
    def test_payment_balance_before_topup(self, project_context):
        ...
        # check event date = 2020-01-01 date, arpu roaming should be 1
        assert \
            daily_payament_balance_before_topup.where("event_partition_date = '2020-01-01'").select(
    ...
    def test_most_popular_topup_channel(self, project_context):
        ...
        assert \
            daily_most_popular_topup_channel.where("event_partition_date = '2020-01-01'").select(
    ...
    def test_topup_hour_and_day(self, project_context):
        ...
        assert \
            daily_topup_hour_and_day.where("event_partition_date = '2020-01-01'").select(
    ...
    def test_time_since_last_topup(self, project_context):
        ...
        assert \
            daily_time_since_last_topup.where("event_partition_date = '2020-01-01'").select("recharge_time").collect()[
                0][
    ...
    def test_topup_channels(self, project_context):
        ...
        assert \
            daily_topup_channels.where("event_partition_date = '2020-01-01'").select(
                "payments_top_ups_by_bank_atm_cdm").collect()[0][
                0] == 1
        ...
        assert \
            daily_topup_channels.where("event_partition_date = '2020-01-01'").select(
                "payments_top_up_vol_by_cash_card").collect()[0][
                0] == 0
        ...
        assert \
            daily_topup_channels.where("event_partition_date = '2020-01-01'").select(
                "payments_top_up_vol_by_epin_slip").collect()[0][
                0] == 0
