...
class SparkDataSet(DefaultArgumentsMixIn, AbstractVersionedDataSet):
    ...
    def _get_metadata_max_data_date(self, spark, table_name):
        ...
        print("spark:", spark)
        print("""select cast( max(to_date(nvl(target_max_data_load_date,'1970-01-01'),'yyyy-MM-dd')) as String) as target_max_data_load_date, 1 as test
            from mdtl where table_name = '{0}' group by test""".format(lookup_table_name))
        target_max_data_load_date = spark.sql(
            """select cast( max(to_date(nvl(target_max_data_load_date,'1970-01-01'),'yyyy-MM-dd')) as String) as target_max_data_load_date, 1 as test
            from mdtl where table_name = '{0}'""".format(lookup_table_name))
        if(target_max_data_load_date.select('target_max_data_load_date').collect()[0][0]==None):
            target_max_data_load_date=spark.sql("select '1970-01-01' as target_max_data_load_date, 1 as test")
        target_max_data_load_date=target_max_data_load_date.drop('test')
        print('debugstart')
        target_max_data_load_date.printSchema()
        target_max_data_load_date.show()
        print('debugend')
        try:
    ...
    def _get_incremental_data(self):
        try:
            ...
            tgt_filter_date_temp = [(row.target_max_data_load_date) for row in target_max_data_load_date.collect()]
    ...
    def _update_metadata_table(self, spark, metadata_table_path, target_table_name, filepath, write_mode, file_format, partitionBy, mergeSchema):
        ...
        """select cast( max(to_date(nvl(target_max_data_load_date,'1970-01-01'),'yyyy-MM-dd')) as String) as target_max_data_load_date, 1 as test
                    from mdtl where table_name = '{0}'"""

        current_target_max_data_load_date = spark.sql(
            "select cast( max(nvl({0},'1970-01-01')) as String) as event_partition_date, 1 as test from curr_target".format(partitionBy))
        if(current_target_max_data_load_date.select('event_partition_date').collect()[0][0]==None):
            current_target_max_data_load_date=spark.sql("select '1970-01-01' as event_partition_date, 1 as '{0}'".format(partitionBy))
        current_target_max_data_load_date=current_target_max_data_load_date.drop('test')
        # metadata_table_update_max_date_temp = current_target_max_data_load_date.rdd.flatMap(lambda x: x).collect()
        metadata_table_update_max_date_temp = [(row.event_partition_date) for row in current_target_max_data_load_date.collect()]
