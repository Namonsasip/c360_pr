...
def l5_music_lift_scoring_gmm(
    ...
    **kwargs,
    ...
    # patch data
    df_master = df_master.withColumn(
        "music_campaign_type", F.lit("GMM_LOVER")
    )
    spark = get_spark_session()
    mlflow_path = "/Shared/music_mlflow/music_upsell_lightgbm"
    if mlflow.get_experiment_by_name(mlflow_path) is None:
    ...
    df_master_scored.createOrReplaceTempView("temp_load_view")
    spark.sql("DROP TABLE IF EXISTS prod_musicupsell.l5_calling_melody_GMM_LOVER")
    spark.sql(
        "CREATE TABLE prod_musicupsell.l5_calling_melody_GMM_LOVER USING DELTA AS SELECT * FROM temp_load_view"
    )
