...
def create_ontop_package_preference(
    ...
    l1_customer_profile_union_daily_feature_full_load = l1_customer_profile_union_daily_feature_full_load.where(
        "event_partition_date > date('2020-06-01')"
    )
    customer_list = (
        l1_customer_profile_union_daily_feature_full_load.groupby(
            "old_subscription_identifier", "register_date"
        )
        .agg(F.count("*").alias("CNT"))
        .selectExpr(
            "old_subscription_identifier", "date(register_date) as register_date"
        )
    )
    date_list = (
    ...
    spark.sql(
        """
    CREATE TEMPORARY VIEW preference_fold
    AS
    SELECT ds2.analytic_id,ds2.activation_date,package_name_report, COUNT(*) as validity, SUM(distributed_daily_spending) as price_per_month, first(mm_data_Speed) as mm_data_Speed,first(mm_types) as mm_types,first(mm_data_type) as mm_data_type,first(preferred_duration) as preferred_duration, SUM(data_sum) as volumn_per_month,first(preferred_data_limit) as preferred_data_limit,  SUM(data_sum)/SUM(number_of_transaction) as volumn_per_day,first(preferred_data_quota) as preferred_data_quota,first(preferred_package_price) as preferred_package_price FROM
    (SELECT * FROM prod.ds2_fold_forward_spending WHERE date(ontop_end_date) >= date('"""
        + lastmonth_start_s
        + """') AND date(ontop_start_date) <= date('"""
        + lastmonth_end_s
        + """') ) ds2
    INNER JOIN
    (SELECT promotion_code, package_name_report,mm_data_Speed,mm_types,mm_data_type,duration as preferred_duration,data_number as preferred_data_limit,data_quota as preferred_data_quota,price_inc_vat as preferred_package_price FROM """
        + prod_table
        + """ ) prods
    ON prods.promotion_code = ds2.promotion_code
    INNER JOIN
    (SELECT analytic_id,register_date as activation_date,data_sum,day_id as date_id FROM prod_delta.dm15_mobile_usage_aggr_prepaid) dm15
    ON dm15.analytic_id = ds2.analytic_id AND dm15.activation_date = ds2.activation_date AND date(ontop_end_date) >= date(dm15.date_id) AND date(ontop_start_date) <= date(dm15.date_id)
    GROUP BY ds2.analytic_id, ds2.activation_date,package_name_report"""
    )

    # Convert start of week date format to timestamp format for window aggregation function
    l1_data_ontop_purchase_daily_spine = l1_data_ontop_purchase_daily.withColumn(
        "weekly_timestamp", F.col("start_of_week").astype("Timestamp").cast("long"),
    )
    # Picking Columns to be Aggregate Weekly
    ...
    # Current Implementation Suggest that we only aggregate for 30, 60, and 90 days of package preferences
    # For any aggregate_period, we will do window aggregated by weekly
    # However, current rangeBetween function doesn't work on date format, so we use long timestamp instead
    # Example: when 30 period is provide, windows function will aggregate from current day and
    # back (30) * 86400(1 Day) = 30 Days in second
    for period in aggregate_periods:
        window_func = (
            Window.partitionBy(
                "old_subscription_identifier", "register_date", "package_name_report"
            )
            .orderBy(F.col("weekly_timestamp"))
            .rangeBetween(
                -(period * 86400), Window.currentRow
            )  # 86400 is the number of seconds in a day
        )

        l1_data_ontop_purchase_daily_spine = l1_data_ontop_purchase_daily_spine.select(
            *(
                l1_data_ontop_purchase_daily_spine.columns
                + [
    ...
    l1_data_ontop_purchase_daily_spine.count()
    l1_data_ontop_purchase_daily_spine.show()
    return df
