...
def dataupsell_lcg_contamination_monitoring():
    ...
    dataupsell_contacted_campaign_lcg = (
        dataupsell_contacted_campaign.join(
            l0_du_pre_experiment5_groups.where("group_name in ('ATL_propensity_CG','ATL_propensity_CG')").selectExpr(
                "old_subscription_identifier as subscription_identifier", "group_name"
            ),
            ["subscription_identifier"],
            "inner",
    ...
    dataupsell_contacted_campaign_lcg.groupby("campaign_child_code","group_name").agg(F.count("*")).show(100)
